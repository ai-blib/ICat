...
class Agent {
   ...
   async noIdentityActor(canisterId:string,IdlFactory):Promise<ActorSubclass>{
    ...
    async ledgerActor(): Promise<ActorSubclass> {
        return this.createActor(ledgerIDL, process.env.REACT_APP_LEDGER_CANISTER_ID);
    };
    async swapStorageActor(): Promise<ActorSubclass> {
        return this.createActor(ledgerIDL, process.env.REACT_APP_SWAP_STORAGE_CANISTER_ID);
    };
