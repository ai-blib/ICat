...
class Swap {
    ...
    async swap(tokenIn: string,
        ...
        try {
            ...
            const call = await (await this.getActor()).swapExactTokensForTokens(parseAmount(amountIn,decimalIn),parseAmount(amountOutMin,decimalIn),[tokenIn,tokenOut],(owner||""),currentTime)
            const call1 = await (await this.getActor()).swapTokensForExactTokens(parseAmount(amountOut,decimalOut),parseAmount(amountInMax,decimalOut),[tokenIn,tokenOut],(owner||""),currentTime)
            return [call,call1]
        } catch (e) {
            console.log(e, 'swap');
            return e;
