...
import {authClient} from "@/utils/getAgent/identity";
 ...
 class Common{
    ...
    async getMetadata(tokenCanisterId) {
        try {
            return await (await this.commonNoIdentityActor(tokenCanisterId)).getMetadata()
        } catch (e) {
            console.log(e, 'balanceOf');
            return e;
        }
     ...
     async getTokenInfo(tokenCanisterId) {
         try {
             return await (await this.getCommonTokenActor(tokenCanisterId)).getTokenInfo()
         } catch (e) {
             console.log(e, 'balanceOf');
             return e;
         }
     ...
     async balanceOf(tokenCanisterId: string) {
         try {
             ...
             return   await (await this.getCommonTokenActor(tokenCanisterId)).balanceOf(owner)
         } catch (e) {
             console.log(e, 'balanceOf');
             return e;
         }
     ...
     async approve(tokenCanisterId: string, spender: string, value: string | number, decimals: number): Promise<any> {
         try {
             const amount = parseAmount(value.toString(), decimals);
             return await (await this.getCommonTokenActor(tokenCanisterId)).approve(Principal.fromText(spender), amount)
         } catch (e) {
     ...
     async allowance(tokenCanisterId: string, spender: string): Promise<any> {
         try {
             const owner = authClient?.principal;
             return await (await this.getCommonTokenActor(tokenCanisterId)).allowance(owner,Principal.fromText(spender))
         } catch (e) {
             console.log(e, 'approve');
             return e;
         }
