...
class Assets {
    ...
    async getUserInfoByNamePageAbove(_owner: Principal):Promise<TokenInfo[]> {
        try {
            const result = await (await this.getActor()).getUserInfoByNamePageAbove(_owner, 0, "", // token name
                0, // token start at
                0, // token limit
                0, // above 0
                "", // lp name or symbol
                0,
                10);
            const res = (result as { lpBalances: [[string, BigInt][], BigInt] }).lpBalances[0];
            if (!res.length){
                return [];
            }
            const info = await Promise.all(res.map((_i) => CommonApi.getTokenInfo(String(_i[0]))))
    ...
    async deposit(tokenCanisterId: string, value: string | number, decimals: number) {
        try {
            const amount = parseAmount(value.toString(), decimals);
            return await (await this.getActor()).deposit(Principal.fromText(tokenCanisterId), amount)
        } catch (e) {
    ...
    async withDraw(tokenCanisterId: string, value: string | number, decimals: number){
        try {
            const amount = parseAmount(value.toString(), decimals);
            debugger;
            return await (await this.getActor()).withdraw(Principal.fromText(tokenCanisterId), amount)
        } catch (e) {
