...
class Liquidity {
    ...
    async getUserLPBalances(owner: Principal) {
        try {
            let result: unknown = await (await this.getActor()).getUserLPBalances(owner);
            if (result) {
                const lps: any[] = [];
    ...
    async getPair(token0: string, token1: string): Promise<TokenPair[] | []> {
        try {
            const result = (await (await this.getActor()).getPair(Principal.fromText(token0) || '', Principal.fromText(token1) || ""))
    ...
    async getTokenInfo(tokenCanisterId) {
        try {
            return await (await this.getCommonTokenActor(tokenCanisterId)).getMetadata()
        } catch (e) {
    ...
    async addLiquidity(token0: string, token1: string, amount0: string, amount1: string, amount0Min: string, amount1Min: string, decimal0: number, decimal1: number) {
        try {
            const currentTime = (new Date().getTime()+5*60*1000) * 10000000;
            return await (await this.getActor()).addLiquidity(Principal.fromText(token0),
    ...
    async removeLiquidity(token0: string, token1: string, lpAmount: string, amount0Min: string, amount1Min: string, decimal0: number|string, decimal1: number|string) {
        try {
            const currentTime = (new Date().getTime()+5*60*1000) * 10000000;
            return await (await this.getActor()).removeLiquidity(Principal.fromText(token0),
                Principal.fromText(token1),
                BigInt(lpAmount),
